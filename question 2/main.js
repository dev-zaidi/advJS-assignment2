// The mobile data object you provided
    var mobiles = {
    iphone: {
      iphone7: {
    ...
    samsung: {
      ...
      samsung_Galaxy_S77: {
        ...
        //brand dropdown
        function brandsOfDevices() {
            var chosenBrand = document.getElementById("chosenBrand");
            for (var brand in mobiles) {
                var option = document.createElement("option");
                var optionText=document.createTextNode(brand);
                option.appendChild(optionText);
                option.setAttribute('value',brand);
                chosenBrand.appendChild(option);
            }
        }

        //model dropdown
        function modelsOfDevices() {
            var chosenBrand = document.getElementById("chosenBrand");
            var chosenModel = document.getElementById("chosenModel");
            var selectedBrand = chosenBrand.value;
            
            // Clear previous model options
            chosenModel.innerHTML = '<option value="">--Select Model--</option>';
            
            
            if (selectedBrand) {
                var modelsObject = mobiles[selectedBrand];
                for (var model in modelsObject) {
                    var option = document.createElement("option");
                    var optionText=document.createTextNode(model);
                    option.appendChild(optionText);
                    option.setAttribute('value',model);
                    chosenModel.appendChild(option);
                }
            }
        }
        ...
        function displayModelDetails() {
            ...
            var selectedBrand = chosenBrand.value;
            var selectedModel = chosenModel.value;

            if (selectedBrand && selectedModel) {
                var model = mobiles[selectedBrand][selectedModel];
                //json.stringify to convert the object in the value to string  
                modelDetails.textContent = JSON.stringify(model,null,2);
            } else {
